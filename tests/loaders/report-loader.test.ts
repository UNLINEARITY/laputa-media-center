...
async function loadReportLoader() {
  vi.resetModules()
  ...
  vi.doMock('@/lib/db/tables/api-calls', () => ({
    findByJobId: vi.fn(() => []),
  }))

  vi.doMock('@/lib/db/tables/job-logs', () => ({
    queryJobLogs: vi.fn(() => []),
  }))

  vi.doMock('@/lib/db/tables/job-scenes', () => ({
    findAllByJobId: vi.fn(() => []),
  }))
  ...
  vi.doMock('@/lib/db/tables/job-videos', () => ({
    findByJobId: vi.fn(() => []),
  }))

  vi.doMock('@/lib/db/tables/scene-audio-candidates', () => ({
    findBySceneId: vi.fn(() => []),
  }))

  vi.doMock('@/lib/exporters/error-analyzer', () => ({
    analyzeJobErrors: vi.fn(() => null),
  }))
  ...
  return import('@/lib/loaders/report-loader')
}
...
describe('report direct loader', () => {
  it('returns provider smoke audit as report evidence', async () => {
    mockedJob = makeJob()
    mockedProviderSmokeAudit = makeProviderSmokeAudit()

    const { loadJobReportDirect } = await loadReportLoader()
    const data = await loadJobReportDirect('job123')

    expect(data?.providerSmokeAudit).toEqual(mockedProviderSmokeAudit)
    expect(data?.providerSmokeDryRunLedger).toMatchObject({
      dry_run_found: true,
      dry_run_checked_at: 1234,
    })
  })

  it('returns null provider smoke audit when no evidence exists', async () => {
    mockedJob = makeJob()
    mockedProviderSmokeAudit = null
