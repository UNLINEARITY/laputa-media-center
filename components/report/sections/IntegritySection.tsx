...
interface IntegritySectionProps {
  ...
  showLegacySceneChecks?: boolean
}

export function IntegritySection({
  integrityCheck,
  stats,
  showLegacySceneChecks = stats.totalScenes > 0,
}: IntegritySectionProps) {
      ...
      <SectionCard title="🔍 数据完整性检查">
        ...
        <div className="mt-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {showLegacySceneChecks ? (
              ...
              <>
                <StatItem label="总日志" value={stats.totalLogs} />
                <StatItem label="错误日志" value={stats.errorLogs} color="red" />
                <StatItem label="警告日志" value={stats.warnLogs} color="gray" />
                <StatItem label="API 调用" value={stats.totalApiCalls} />
              </>
            )}
          </div>
        </div>
        ...
        <div className="mt-4 space-y-3">
          ...
          <CheckGroup title="API 调用统计">
            <div
              className={`grid gap-2 text-xs ${showLegacySceneChecks ? 'grid-cols-3' : 'grid-cols-1'}`}
            >
              {showLegacySceneChecks && (
                <>
