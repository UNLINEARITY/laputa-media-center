...
YouTube / 网页视频读取失败。请确认 yt-dlp 可用；如 YouTube 要求登录，请配置 cookies。`
  ...
  if (normalized.includes('spawn whisper') || normalized.includes('no module named whisper')) {
    return `${message}
Whisper 未就绪。可设置 INGEST_PYTHON_EXE，或复用已可用的 DUBBING_PYTHON_EXE。`
  }
...
ffmpeg 未就绪。可设置 INGEST_FFMPEG_EXE，或复用已可用的 DUBBING_FFMPEG_EXE。`
  ...
  return `${message}
请确认已安装 ffmpeg、yt-dlp（YouTube 时需要）和 Whisper；或设置 INGEST_FFMPEG_EXE、INGEST_YTDLP_EXE、INGEST_PYTHON_EXE、INGEST_WHISPER_CLI。`
}
...
export class TranscribeMediaStep extends BaseStep {
  ...
  getInputSummary(ctx: WorkflowContext): Record<string, unknown> {
    ...
    return {
      ...
      whisper_runtime:
        process.env.INGEST_PYTHON_EXE || process.env.DUBBING_PYTHON_EXE
          ? 'python -m whisper'
          : process.env.INGEST_WHISPER_CLI
            ? 'configured whisper cli'
            : 'PATH:whisper',
      whisper_model:
        process.env.INGEST_WHISPER_MODEL || process.env.DUBBING_WHISPER_MODEL || 'base',
    }
  }
