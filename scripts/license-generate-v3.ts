...
function parseFeatures(featuresStr: string): number {
  ...
  const normalized = features.map((feature) => LEGACY_FEATURE_ALIASES[feature] || feature)

  if (normalized.includes(AVAILABLE_FEATURES.CONTENT_INGEST) || features.includes('1')) {
    flags |= 0b001
  }
  if (normalized.includes(AVAILABLE_FEATURES.TRANSLATION_DUBBING) || features.includes('2')) {
    flags |= 0b010
  }
  if (normalized.includes(AVAILABLE_FEATURES.CREATOR_ASSETS) || features.includes('4')) {
    flags |= 0b100
  }
...
function main() {
  ...
  if (/^\d+$/.test(featuresInput)) {
    ...
    // 逗号分隔格式（如 content_ingest,translation_dubbing；旧名称仅作为输入别名）
  ...
  if (features & 0b001) featureNames.push('素材吸收')
  if (features & 0b010) featureNames.push('翻译配音')
  if (features & 0b100) featureNames.push('创作者资产')
