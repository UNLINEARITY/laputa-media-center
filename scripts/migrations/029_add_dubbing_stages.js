/**
 * Migration 029: allow translation dubbing stage IDs in SQLite CHECK constraints.
 */
...
const STEP_VALUES =
  "'analysis', 'generate_narrations', 'extract_scenes', 'process_scenes', 'compose', 'asr', 'translate', 'voiceclone', 'lipsync', 'ingest', 'transcribe', 'package'"
...
function migrateJobs(db) {
  ...
  if (sql.includes("'voiceclone'") && sql.includes("'lipsync'")) {
    console.log('[Migration 029] jobs.current_step constraint already updated')
    return
  }

  console.log('[Migration 029] rebuilding jobs table')
  db.prepare(`
    CREATE TABLE jobs_new (
      id TEXT PRIMARY KEY,
      status TEXT NOT NULL CHECK(status IN ('pending', 'processing', 'completed', 'failed')),
      current_step TEXT CHECK(current_step IN (${STEP_VALUES})),
...
function migrateStepHistory(db) {
  ...
  if (sql.includes("'voiceclone'") && sql.includes("'lipsync'")) {
    console.log('[Migration 029] job_step_history.major_step constraint already updated')
    return
  }

  console.log('[Migration 029] rebuilding job_step_history table')
  db.prepare(`
    CREATE TABLE job_step_history_new (
      ...
      major_step TEXT NOT NULL CHECK(major_step IN (${STEP_VALUES})),
...
function migrate() {
  console.log('[Migration 029] start')
