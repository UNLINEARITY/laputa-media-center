...
**最後更新**：2026-05-05（每次對話結束 AI 助手會更新這裡）
...
**下次從哪裡繼續**：本輪已把 LLM 翻譯改成可配置通用 Base URL + API Key，支持 OpenAI-compatible 與 Claude / Anthropic request format；TTS 端保留 MiniMax 主線與合規聲線系統，但 MiniMax-compatible endpoint 可配置 Base URL；並修正 dev 模式用 `127.0.0.1:8899` 打開 Settings 時 hydration 被 Next dev origin gate 阻斷的問題。下一步優先做桌面首啟配置嚮導與小白文檔，把「無 API key 可先跑免費/占位體驗、有 key 再升級」講清楚。
...
- 2026-05-05 Settings dev origin 修正：`next.config.ts` 的 `allowedDevOrigins` 補入 `127.0.0.1` 和 `localhost`，避免用 `http://127.0.0.1:8899/settings` 打開時 Next.js 16 dev server 拒絕 HMR/font dev resource，導致 React hydration 未完成、Provider 列表和前置條件面板一直停在 loading。已重啟 dev server，Playwright/Edge headless 分別驗證 `http://127.0.0.1:8899/settings` 與 `http://localhost:8899/settings`：`/api/providers/asr`、`/api/providers/llm`、`/api/setup-requirements` 等請求正常返回，頁面不再卡 loading；`corepack pnpm lint`、`corepack pnpm typecheck:app` 通過。
...
- 2026-05-04 實機部署抽查：本地 `pnpm build` 通過，`LMC_BYPASS_LICENSE=true AUTH_ENABLED=false PORT=8899 pnpm start` 可打開首頁、Settings、Ingest、Podcast、Highlights；補回缺失的 `/api/jobs/[id]/logs` route，`pnpm test:unit` 107 files / 806 pass / 17 skip / 0 fail。後續仍需收斂開源體驗點：README 說無 `LICENSE_KEY` 可直接本地跑，但 production proxy 仍需 `LMC_BYPASS_LICENSE=true`。
...
- 分發前置鏈路簡化：Lite 便攜運行時已能讀隨包 resources；剩餘重點是把 ffmpeg / yt-dlp / whisper.cpp 模型從「占位或本機已有才複製」推進到「一鍵下載 / 隨包攜帶 / 首次啟動自檢」，再評估是否打包成 Tauri / Windows installer。
...
**Phase 0 commit**：`9de940e` chore: Phase 0 — 從 ChuangCut 重構為 LaputaMediaCenter（636 文件，144685 行）
...
### Phase 1：砍 SaaS 偽裝 ✅ 完成（2026-04-30，commits `a89f352` + `ca6c3db` + 收尾）
...
2. Git history rewrite 清舊工作流帳號殘留（已完成；保留 `hkdadinsz*` / `laputa`）
3. 寫 README.md（中文 + 英文）
...
**C. 標題/封面建議 + 開頭鉤子優化器** ✅ 完成（commit `bb5bba0`）
...
### Phase 0：項目初始化 ✅ 完成（2026-04-30，commit `9de940e`）
- [x] 從前身項目選擇性複製代碼
...
- [x] **W0**：TS production errors 全修 + biome lint 全綠（commit `5086379`）
- [x] **W1**：Mandarin prompt 分支補齊 + /title-hooks cost 文案修正（commit `da26ac5`）
- [x] **W2**：4 工具 nav + 首頁 dashboard + AUTH-aware login + /api/health + MD/PDF 切換清空 + podcast 無聲線 CTA + UI 術語去工程化（commit `1437bd7`）
- [x] **W3**：9:16 ffmpeg scale+pad+setsar + preview aspect + settings 404 silent + mobile chip 32px + 品牌統一（site-logo / footer / license-error / README）+ CODEX_HANDOFF.md 受保護資產路徑修正 + CHANGELOG.md 起步（commit `e2dc1e7`）
...
- [x] **Wave 5.1 secrets 全項目掃描完成**（commit `8d771ca` B+ 脫敏）:
  ...
  - **個資 5 類**（非 security incident，是隱私決策）：
    ...
    2. 舊工作流 Gmail 曾在已刪除的 docs/agent/credentials.md（rewrite 後該路徑已從 history 抹除；不要在 repo 再寫成連續明文）
    3. Zeabur project IDs 曾在 history 中（rewrite 後已 scrub）
    4. 舊工作流 GitHub username 曾在 deleted history（rewrite 後已 scrub；PAT 是 `ghp_xxx` 占位符不是實值）
    5. Cloudflare R2 public bucket URL（intentionally 公開，無風險）
  - **採 B+ 方案並完成 history rewrite**：
...
- [x] **Git history rewrite 清舊工作流帳號殘留完成**（commit `a8eb924`；保留 `hkdadinsz*` / `laputa`；mirror backup：`../LaputaMediaCenter.pre-history-rewrite.git`）
...
- [x] **Tool catalog normal form 收斂**:
  - 新增 `lib/product/tool-catalog.ts`，把 Header 工具下拉與首頁 dashboard 兩套並行工具分類收斂到同一份 8-entry catalog
  - selector 採 predicate filter，不在 entry 上加 `surfaces` flag；presentation icon / layout 仍留在 dashboard adapter
...
- **2026-04-30**：Phase 0 完成（commit `9de940e`）。代碼骨架從 ChuangCut 拷貝就位，受保護資產原封不動，dev server 跑通。proxy.ts 加 dev mode license bypass（保留 V3 機制）。下一步進 Phase 1。
