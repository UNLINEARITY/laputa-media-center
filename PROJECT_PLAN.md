...
**下次從哪裡繼續**：✅ Codex 兩輪所有 issue 全修完（commit `512b330` `84007c0` `563dd77` `8caa7b5` `a6f93c3`）。**主線：進 Phase 5 開源就緒**（secrets 全項目掃描 / history rewrite 清舊工作流帳號殘留 / README EN/ZH / install scripts / demo GIF / GitHub push）。
...
- [x] **Wave 5.1 secrets 全項目掃描完成**（commit `8d771ca` B+ 脫敏）:
  ...
  - **個資 5 類**（非 security incident，是隱私決策）：
    ...
    2. 舊工作流 Gmail 在 commits `9de940e` / `a89f352` 已刪除的 docs/agent/credentials.md（在 history 仍可 git show；不要在 repo 再寫成連續明文）
